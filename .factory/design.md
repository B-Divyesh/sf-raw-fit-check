...
- `site/public/raw-bench.webp`: original generated hero still life—an opened RAW file cartridge, camera memory card, contact-sheet crop marks, and a small diagnostic oscilloscope trace. It explains “inspect the file before the workflow.” Generated for this product with `/opt/fleet/lib/gen-image.sh`, factory-image model, then converted locally to WebP. Prompt: “Neo-brutalist editorial still life for a RAW photo compatibility diagnostic; overhead view of an unbranded black camera memory card, a cream contact sheet with crop marks, a small waveform/diagnostic strip, hard ink outlines, warm paper, inspection yellow and cobalt blue, coarse screenprint texture, asymmetrical horizontal composition, no people, no logos, no readable text, no gradients, generous negative space.” Original generation; project use.
